"""List tasks from the Camunda API."""
...
def list_tasks(
    ...
    columns: list[str] | None = typer.Option(
        ...
        "-c",
        help=(
            "Comma-separated task columns to display. "
            f"Available: {', '.join(task_column_names())}."
        ),
    ),
    output: Path | None = typer.Option(
        ...
        "--output",
        "-o",
        help="Write the task list response to a JSON file.",
        dir_okay=False,
        resolve_path=True,
    ),
    ...
    with context.build_engine() as engine:
        page_result = engine.tasks.list(params=params)
    ...
    if format is OutputFormat.JSON:
        print_json(page_result, title="Tasks")
        return

    selected_columns = parse_comma_list(columns)
    try:
        print_tasks(page_result, columns=selected_columns)
