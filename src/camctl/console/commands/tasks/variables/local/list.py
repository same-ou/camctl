...
@local_app.command(
    "list",
    help="List local variables for a task.",
    short_help="List local variables.",
)
def list_local_variables(
    ...
    """Retrieve local variables for a task and print the JSON response."""
    context = require_context(ctx)
    with context.build_engine() as engine:
        payload = engine.tasks.list_local_variables(
            task_id,
    ...
    if raw:
        ...
        print_json(payload, title="Local Task Variables")
