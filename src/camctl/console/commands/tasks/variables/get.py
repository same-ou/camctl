...
@variables_app.command(
    "get",
    help="Fetch a single task variable.",
    short_help="Get a variable.",
)
def get_variable(
    ctx: typer.Context,
    ...
    """Retrieve a single task variable and print the JSON response."""
    context = require_context(ctx)
    with context.build_engine() as engine:
        payload = engine.tasks.get_variable(
            task_id,
            var_name,
    ...
    if raw:
        ...
        print_json(payload, title=f"Variable {var_name}")
